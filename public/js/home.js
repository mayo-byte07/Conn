...
(function () {
  ...
  function initContactForm() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!inquiryType) return showError("Please select an inquiry type.");
      if (!name || name.length < 2)
        return showError("Please enter a valid name (min 2 characters).");
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
        return showError("Please enter a valid email address.");
      if (!message || message.length < 20)
        return showError("Message must be at least 20 characters long.");
      ...
      try {
        const res = await fetch("/api/contact", {
          ...
          body: JSON.stringify({ inquiryType, name, email, message }),
        });
        ...
        if (res.ok && data.success) {
          ...
          setTimeout(resetContactForm, 8000);
        } else {
          showError(
  ...
  function initRazorpayLanding() {
    ...
    btnPro.addEventListener("click", (e) =>
      handleUpgradeClick(e, "professional"),
    );
  }
  ...
  function initCustomCursor() {
    ...
    const interactables = document.querySelectorAll(
      "a, button, .feature-card, .pricing-card, .use-case-card, .theme-preview-card",
    );
    ...
    if (typeof VanillaTilt !== "undefined") {
      VanillaTilt.init(
        document.querySelectorAll(
          ".feature-card, .use-case-card, .pricing-card, .testimonial-card, .learn-card",
        ),
        {
          max: 8,
          speed: 400,
